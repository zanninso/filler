...
/*   Updated: 2019/12/22 21:23:43 by aait-ihi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "filler_visu.h"

static void put_pixel(t_visu *visu, int x, int y, int color)
{
...
void draw_percentag(t_visu *visu)
{
    ...
    if (visu->x_piece == 0 || visu->o_piece == 0)
        return;
    p1_percent = (float)visu->o_piece * 100 / (float)(visu->x_piece + visu->o_piece);
    p1 = (t_point){visu->percent_start_x, visu->start_menu};
    p2 = (t_point){p1.x + (visu->percent_width / 100.) * p1_percent, visu->start_menu + 50};
    draw_rectangle(p1, p2, WHITH, visu);
    p1 = (t_point){p2.x, visu->start_menu};
    p2 = (t_point){visu->percent_start_x + visu->percent_width, visu->start_menu + 50};
...
void draw_menu(t_visu *visu)
{
    ...
    if (visu->round == 0)
    {
        ...
        draw_rectangle((t_point){0, 0}, (t_point){visu->win_width, visu->start_y}, 0x2a2d33, visu);
    }
    if (visu->round % 2)
        draw_percentag(visu);
